...
class Input():
    def __init__(self, debug = False):
        ...
        self.capture = cv2.VideoCapture("/home/treenulbo/Develop/live-dance-tracker/youtube_example/BTS_GO.mp4")
        ...
        self.out = cv2.VideoWriter('dance_tracker_person_1.avi', fourcc, 30.0, (Constants.SCREEN_WIDTH, Constants.SCREEN_HEIGHT))
    ...
    def color_distance(self, color_1, color_2):
        original_srgb = sRGBColor(color_2[0], color_2[1], color_2[2], is_upscaled=True)
        new_srgb = sRGBColor(color_1[0], color_1[1], color_1[2], is_upscaled=True)
    ...
    def min_color_distance(self, new_color, original_list):
        new_srgb = sRGBColor(new_color[0], new_color[1], new_color[2], is_upscaled=True)
        new_color = convert_color(new_srgb, LabColor)
        min_key = 0
        min_delta = 100000

        for index in original_list:
            original_srgb = sRGBColor(original_list[index][0], original_list[index][1], original_list[index][2], is_upscaled=True)
            original_color = convert_color(original_srgb, LabColor)
    ...
    def cord_distance(self, original_cords, new_cords):
        formula_1 = pow((original_cords[0] - new_cords[0]), 2) + pow((original_cords[1] - new_cords[1]), 2)
        formula_2 = pow((original_cords[2] - new_cords[2]), 2) + pow((original_cords[3] - new_cords[3]), 2)
    ...
    def run(self, p_id, col_list, g_num, start, end):
        ...
        poses = keypoints[:,[2, 3, 5, 6, 8, 9, 10, 11, 12, 13, 14],:2]  # [RShoulder, RElbow, LShoulder, LElbow, MidHip, RHip,Rknee, RAnkle, LHip, LKnee, LAnkle]
        ...
        detections = [Detection(bbox, 1.0, feature, pose) for bbox, feature, pose in zip(boxesxywh, features, poses) if nonempty(bbox)]
        ...
        for track in track_list:
            ...
            obj_box = [int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])]  # object tracker cordinates
            col_box = [int(bbox[0])-2, int(bbox[1])+2, int(bbox[2])+2, int(bbox[1])+2 - int((int(bbox[1]) + 2 - int(bbox[3])) * 0.6)] # color tracker cordinates
            ...
            try:
                ...
                if clear_condition == True:
                    dominant_color = self.image_color_cluster(template, person_key)
                    if len(col_list) == 0:
                        col_list[person_key] = dominant_color    
                    elif person_key in list(col_list.keys()):
                        if self.color_distance(dominant_color, col_list[person_key]) < 5: # 안가려져있을때
                            col_list[person_key] = dominant_color
                        else:
            ...
            if (col_box[2] - col_box[0]) > 40 and (col_box[2] - col_box[0]) < 220:
                ...
                cv2.rectangle(self.currentFrame, (obj_box[0], obj_box[1]), (obj_box[2], obj_box[3]),color, 2)
                ...
                if person_key in key_list and color_detected == True:
                    cv2.rectangle(self.currentFrame, (col_box[0], col_box[1]), (col_box[2], col_box[3]), (col_list[person_key][2], col_list[person_key][1], col_list[person_key][0]), 2) #Drawing a green rectangle around the template

                # Allocate Id
                cv2.putText(self.currentFrame, "id%s"%(person_key),(obj_box[0], obj_box[1]-20), 0, 5e-3 * 200, (0,255,0), 2)
